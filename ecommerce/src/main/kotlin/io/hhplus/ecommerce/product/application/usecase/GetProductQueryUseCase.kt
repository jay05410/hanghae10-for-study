...
class GetProductQueryUseCase(
    ...
    /**
     * 상품 목록을 커서 기반 페이징으로 조회한다 - Redis 캐시 적용
     * TTL: 5분 (상품 목록 일관성 유지)
     *
    ...
    fun getProducts(lastId: Long?, size: Int): Cursor<Product> {
        return productDomainService.getProductsWithCursor(lastId, size)
    }

    /**
     * 상품 ID로 특정 상품을 조회한다 - 로컬 캐시 적용
    ...
    /**
     * 특정 카테고리에 속하는 상품 목록을 커서 기반 페이징으로 조회한다 - Redis 캐시 적용
    ...
    fun getProductsByCategory(categoryId: Long, lastId: Long?, size: Int): Cursor<Product> {
        return productDomainService.getProductsByCategoryWithCursor(categoryId, lastId, size)
    }
    ...
    fun getProductStatistics(productId: Long): ProductStatsVO {
        val (viewCount, salesCount, wishCount) = productStatisticsPort.getRealTimeStats(productId)
    ...
    fun getPopularStatistics(limit: Int = 10): List<ProductStatsVO> {
        return productStatisticsPort.getRealTimePopularProducts(limit).map { (productId, viewCount) ->
            val (_, salesCount, wishCount) = productStatisticsPort.getRealTimeStats(productId)
