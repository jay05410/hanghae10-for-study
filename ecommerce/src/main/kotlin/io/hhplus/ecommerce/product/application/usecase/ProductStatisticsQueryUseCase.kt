...
class ProductStatisticsQueryUseCase(
    ...
    fun getProductsByCategoryAndSortCriteria(
        ...
        // 1. 해당 카테고리의 활성 상품 조회
        val categoryProducts = productDomainService.getActiveProductsByCategory(categoryId)

        // 2. 정렬 기준에 따른 필터링 및 정렬
        return when (sortBy) {
            ProductSortCriteria.POPULAR -> {
                categoryProducts.map { product ->
                    val (viewCount, salesCount, wishCount) = productStatisticsPort.getRealTimeStats(product.id)
            ...
            ProductSortCriteria.MOST_VIEWED -> {
                categoryProducts.map { product ->
                    val viewCount = productStatisticsPort.getLast10MinuteViews(product.id)
            ...
            ProductSortCriteria.MOST_WISHED -> {
                categoryProducts.map { product ->
                    val (_, _, wishCount) = productStatisticsPort.getRealTimeStats(product.id)
            ...
            ProductSortCriteria.BEST_SELLING -> {
                categoryProducts.map { product ->
                    val (_, salesCount, _) = productStatisticsPort.getRealTimeStats(product.id)
    ...
    private fun getPopularProducts(limit: Int): List<Product> {
        val popularProductIds = productStatisticsPort.getRealTimePopularProducts(limit)
    ...
    private fun getMostViewedProducts(limit: Int): List<Product> {
        ...
        for (productId in 1L..1000L) {
            try {
                val viewCount = productStatisticsPort.getLast10MinuteViews(productId)
                if (viewCount > 0) {
    ...
    private fun getMostWishedProducts(limit: Int): List<Product> {
        ...
        for (productId in 1L..1000L) {
            try {
                val (_, _, wishCount) = productStatisticsPort.getRealTimeStats(productId)
                if (wishCount > 0) {
    ...
    private fun getBestSellingProducts(limit: Int): List<Product> {
        ...
        for (productId in 1L..1000L) {
            try {
                val (_, salesCount, _) = productStatisticsPort.getRealTimeStats(productId)
                if (salesCount > 0) {
