...
interface PaymentHistoryJpaRepository : JpaRepository<PaymentHistoryJpaEntity, Long> {
     ...
     * 결제 ID로 이력 목록 조회
     */
    fun findByPaymentId(paymentId: Long): List<PaymentHistoryJpaEntity>

    /**
     * 결제 ID로 이력 목록 조회 (생성 시간 내림차순)
     */
    fun findByPaymentIdOrderByCreatedAtDesc(paymentId: Long): List<PaymentHistoryJpaEntity>
    ...
    fun findLatestByPaymentId(paymentId: Long): List<PaymentHistoryJpaEntity>
}
