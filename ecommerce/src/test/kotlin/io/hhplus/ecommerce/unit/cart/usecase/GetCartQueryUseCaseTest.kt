 ...
 *
 * 검증 목표:
 * 1. CartDomainService에 올바른 파라미터가 전달되는가?
 * 2. 서비스 결과가 그대로 반환되는가?
 * 3. 존재하지 않는 장바구니에 대한 처리가 올바른가?
 */
class GetCartQueryUseCaseTest : DescribeSpec({
    val mockCartDomainService = mockk<CartDomainService>()
    val sut = GetCartQueryUseCase(mockCartDomainService)

    beforeEach {
        clearMocks(mockCartDomainService)
    }

    describe("execute") {
        context("장바구니가 존재하는 사용자") {
            it("CartDomainService에 조회를 위임하고 장바구니를 반환") {
                val userId = 1L
                val expectedCart = mockk<Cart>()

                every { mockCartDomainService.getCartByUser(userId) } returns expectedCart

                val result = sut.execute(userId)

                result shouldBe expectedCart
                verify(exactly = 1) { mockCartDomainService.getCartByUser(userId) }
            }
        }

        context("장바구니가 없는 사용자") {
            it("CartDomainService에 조회를 위임하고 null을 반환") {
                val userId = 999L

                every { mockCartDomainService.getCartByUser(userId) } returns null
        ...
        context("다양한 사용자 ID로 조회") {
            it("각각의 사용자 ID를 정확히 서비스에 전달") {
                ...
                userIds.forEachIndexed { index, userId ->
                    every { mockCartDomainService.getCartByUser(userId) } returns mockCarts[index]
        ...
        context("연속된 조회 요청") {
            it("매번 서비스를 호출하고 결과를 반환") {
                ...
                every { mockCartDomainService.getCartByUser(userId) } returnsMany listOf(mockCart1, mockCart2)
                ...
                verify(exactly = 2) { mockCartDomainService.getCartByUser(userId) }
            }
        }
    }
})