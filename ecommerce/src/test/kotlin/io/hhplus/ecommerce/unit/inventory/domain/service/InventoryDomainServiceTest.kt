package io.hhplus.ecommerce.unit.inventory.domain.service
...
/**
 * InventoryDomainService 단위 테스트
 *
 * 책임: 재고 도메인 서비스의 핵심 비즈니스 로직 검증
 * - 재고 생성, 차감, 충당, 예약 기능 검증
 * - Repository와의 상호작용 검증
 */
class InventoryDomainServiceTest : DescribeSpec({
    val mockInventoryRepository = mockk<InventoryRepository>()
    val sut = InventoryDomainService(mockInventoryRepository)
    ...
    describe("getInventoryOrThrow") {
        context("존재하는 상품 재고 조회") {
            it("Repository에서 재고를 조회하여 반환") {
                ...
                val result = sut.getInventoryOrThrow(productId)
        ...
        context("존재하지 않는 상품 재고 조회") {
            it("InventoryNotFound 예외를 발생") {
                ...
                shouldThrow<InventoryException.InventoryNotFound> {
                    sut.getInventoryOrThrow(productId)
                }
