    ...
    describe("배송 생성") {
        ...
        context("동일 주문에 대해 중복 배송 생성 시도할 때") {
            it("예외가 발생한다") {
                ...
                deliveryService.createDelivery(orderId, deliveryAddress, null, 1L)
    ...
    describe("배송 상태 변경") {
        context("배송 준비 시작 시") {
            it("상태가 PREPARING으로 변경된다") {
                ...
                val delivery = deliveryService.createDelivery(orderId, deliveryAddress, null, 1L)

                // When
                val updated = deliveryService.startPreparing(delivery.id, 1L)
        ...
        context("배송 발송 처리 시") {
            it("상태가 SHIPPED로 변경되고 운송장 번호가 기록된다") {
                ...
                val delivery = deliveryService.createDelivery(orderId, deliveryAddress, null, 1L)
                deliveryService.startPreparing(delivery.id, 1L)

                // When
                val trackingNumber = "1234567890"
                val carrier = "CJ대한통운"
                val updated = deliveryService.ship(delivery.id, trackingNumber, carrier, 1L)
        ...
        context("배송 완료 처리 시") {
            it("상태가 DELIVERED로 변경된다") {
                ...
                val delivery = deliveryService.createDelivery(orderId, deliveryAddress, null, 1L)
                deliveryService.startPreparing(delivery.id, 1L)
                deliveryService.ship(delivery.id, "9876543210", "롯데택배", 1L)

                // When
                val updated = deliveryService.deliver(delivery.id, 1L)
        ...
        context("배송 실패 처리 시") {
            it("상태가 FAILED로 변경된다") {
                ...
                // When
                val updated = deliveryService.fail(delivery.id, 1L)
    ...
    describe("배송 전체 플로우") {
        context("PENDING → PREPARING → SHIPPED → DELIVERED 순서로 진행 시") {
            it("정상적으로 배송이 완료된다") {
                ...
                // When - PENDING
                val delivery = deliveryService.createDelivery(orderId, deliveryAddress, "빠른 배송 부탁드립니다", 1L)
                delivery.status shouldBe DeliveryStatus.PENDING

                // When - PREPARING
                val preparing = deliveryService.startPreparing(delivery.id, 1L)
                preparing.status shouldBe DeliveryStatus.PREPARING

                // When - SHIPPED
                val shipped = deliveryService.ship(delivery.id, "TRACK123", "한진택배", 1L)
                shipped.status shouldBe DeliveryStatus.SHIPPED

                // When - DELIVERED
                val delivered = deliveryService.deliver(delivery.id, 1L)
                delivered.status shouldBe DeliveryStatus.DELIVERED

                // Then
                val final = deliveryService.getDelivery(delivery.id)
    ...
    describe("배송 조회") {
        context("배송 ID로 조회할 때") {
            it("배송 정보를 조회할 수 있다") {
                ...
                // When
                val found = deliveryService.getDelivery(delivery.id)
        ...
        context("주문 ID로 조회할 때") {
            it("해당 주문의 배송을 조회할 수 있다") {
                ...
                // When
                val found = deliveryService.getDeliveryByOrderId(orderId)
        ...
        context("존재하지 않는 주문 ID로 조회할 때") {
            it("예외가 발생한다") {
                ...
                shouldThrow<DeliveryException.DeliveryNotFoundByOrder> {
                    deliveryService.getDeliveryByOrderId(99999L)
                }
            }
        }
        ...
        context("운송장 번호로 조회할 때") {
            it("배송 정보를 조회할 수 있다") {
                ...
                deliveryService.ship(delivery.id, trackingNumber, "우체국택배", 1L)

                // When
                val found = deliveryService.getDeliveryByTrackingNumber(trackingNumber)
        ...
        context("배송 상태로 조회할 때") {
            it("해당 상태의 배송 목록을 조회할 수 있다") {
                ...
                val delivery1 = deliveryService.createDelivery(11001L, deliveryAddress, null, 1L)
                val delivery2 = deliveryService.createDelivery(11002L, deliveryAddress, null, 1L)
                deliveryService.startPreparing(delivery1.id, 1L)

                // When
                val preparingDeliveries = deliveryService.getDeliveriesByStatus(DeliveryStatus.PREPARING)
                val pendingDeliveries = deliveryService.getDeliveriesByStatus(DeliveryStatus.PENDING)
