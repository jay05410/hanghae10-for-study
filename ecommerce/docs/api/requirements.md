...
| ID | 요구사항 | 우선순위 | 설명 |
|----|---------|---------|------|
| FR-001 | 박스 타입 조회 | 높음 | 다양한 기간별 박스 타입 제공 |
| FR-002 | 차 아이템 조회 | 높음 | 카테고리별 차 아이템 목록 제공 |
| FR-003 | 아이템별 재고 조회 | 높음 | 실시간 차 아이템 재고 수량 확인 |
| FR-004 | 일일 생산 한도 체크 | 높음 | 박스타입별 일일 쵝 생산량 제한 |
| FR-005 | 인기 아이템 통계 | 높음 | 최근 7일 TOP 10 아이템 조회 |
...
| ID | 요구사항 | 우선순위 | 설명 |
|----|---------|---------|------|
| FR-009 | 장바구니 추가 | 높음 | 박스타입 선택 및 차 커스텀 장바구니 추가 |
| FR-010 | 장바구니 조회 | 높음 | 현재 장바구니 내역 및 아이템 재고 상태 |
| FR-011 | 장바구니 삭제 | 높음 | 특정 항목 또는 전체 삭제 |
| FR-012 | 여러 박스 담기 | 중간 | 다른 박스타입의 커스텀 박스 여러 개 담기 |
...
### 4.1 상품 규칙
- **BR-001**: 박스 타입은 기간별로 다양하게 제공 (7일, 14일, 30일 등)
- **BR-002**: 각 박스는 당일 수만큼의 차 아이템으로 구성
- **BR-003**: 차 아이템은 카테고리별로 분류 (허브티, 그린티, 블랙티, 화이트티, 루이보스 등)
- **BR-004**: 사용자는 각 날짜에 원하는 차 아이템을 선택 가능
...
- **BR-011**: 동일 박스타입의 커스텀 박스는 장바구니에 1개만 담기 가능
