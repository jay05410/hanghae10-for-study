...
- **규칙**:
  - 결제 금액 > 현재 잔액 → `InsufficientBalance` 예외 발생
  - 결제 금액 <= 현재 잔액 → 정상 처리
- **예외 메시지**: "잔액이 부족합니다. 현재잔액: {currentBalance}, 결제금액: {paymentAmount}"

#### 4.2 포인트 충전 제한
- **정책**: 포인트 충전은 1,000원 이상, 100,000원 이하, 100원 단위만 가능
- **규칙**:
  - 충전 금액 < 1,000원 → `MinimumChargeAmount` 예외 발생
  - 충전 금액 > 100,000원 → `MaximumChargeAmount` 예외 발생
  - 충전 금액 % 100 != 0 → `InvalidChargeUnit` 예외 발생
- **예외 메시지**:
  - 최소: "최소 충전 금액은 1,000원입니다. 요청금액: {amount}"
  - 최대: "최대 충전 금액은 100,000원입니다. 요청금액: {amount}"
  - 단위: "포인트 충전은 100원 단위로만 가능합니다. 요청금액: {amount}"
